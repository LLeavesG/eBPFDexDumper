...
int uprobe_libart_execute(struct pt_regs *ctx)
{
    ...
    if(begin != 0 && size != 0) {
        ...
        struct event_data_t *evt_ptr = bpf_ringbuf_reserve(&events, sizeof(struct event_data_t), 0);
        if (evt_ptr) {
            evt_ptr->begin = begin;
            evt_ptr->pid = pid;
            evt_ptr->size = size;
        ...
        struct method_event_data_t *method_evt = bpf_ringbuf_reserve(&method_events, sizeof(struct method_event_data_t), 0);
        if (method_evt) {
...
int uprobe_libart_executeNterpImpl(struct pt_regs *ctx)
{
    ...
    if(begin != 0 && size != 0) {
        ...
        if (value == 0 || *value != 1){
            // Dex文件未缓存，发送Dex文件事件
            struct event_data_t *dex_evt = bpf_ringbuf_reserve(&events, sizeof(struct event_data_t), 0);
            if (dex_evt) {
                dex_evt->begin = begin;
                dex_evt->pid = pid;
                dex_evt->size = size;
